				...
				<Card.Content>
					{#if error}
						<Alert.Root variant="destructive" class="mb-4">
							...
							<Alert.Description>
								{signinErrors[error as SignInPageErrorParam] ?? signinErrors.default}
							</Alert.Description>
						</Alert.Root>
					{/if}
						...
						{#each data.providerMap as provider}
							...
							>
								...
								{:else if provider.id === 'github'}
									<img src={githubLight} alt="GitHub" class="h-4 w-4 not-dark:hidden" />
									<img src={githubDark} alt="GitHub" class="h-4 w-4 dark:hidden" />
